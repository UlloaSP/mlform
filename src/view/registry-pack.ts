...
import { cloneSchemaRegistry } from "./registry-clone";
import type { MLFormPlugin } from "./plugin";

export interface ViewRegistryPack {
  registry: Registry;
  descriptorRegistry: PrimitiveDescriptorRegistry;
  behaviors: RuntimeBehavior[];
}

export interface ResolveViewRegistryPackOptions {
  registry?: Registry;
  descriptorRegistry?: PrimitiveDescriptorRegistry;
  behaviors?: RuntimeBehavior[];
...
const applyPlugin = (pack: ViewRegistryPack, plugin: MLFormPlugin): void => {
...
const createDefaultRegistryPack = (): ViewRegistryPack => {
  return {
...
export const resolveViewRegistryPack = (
  options: ResolveViewRegistryPackOptions,
): ViewRegistryPack => {
  const defaults = createDefaultRegistryPack();
  const pack: ViewRegistryPack = {
