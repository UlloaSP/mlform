...
import type { FormViewNavigationController } from "./view-core-types";
import type { ResolvedFormLayoutNode } from "./layout-types";
...
const filterOpenSections = (
  ...
  nodes.flatMap<ResolvedFormLayoutNode>((node) => {
    if (node.kind === "section") {
      return openSectionIds.has(node.id)
        ? [{ ...node, children: filterOpenSections(node.children, openSectionIds) }]
        : [];
    }
    if (node.kind === "group") {
      return [{ ...node, children: filterOpenSections(node.children, openSectionIds) }];
    }
    return [node];
  });
...
export const assertDisclosureSection = (
  ...
  if (!disclosureSections.some((section) => section.id === sectionId)) {
    throw new TypeError(viewErrorMessages.unknownDisclosureSection(sectionId));
  }
};
...
export const createFormViewNavigation = ({
  ...
  const activateWizardStep = async (stepId: string): Promise<boolean> => {
    ...
    if (targetIndex < 0) throw new TypeError(viewErrorMessages.unknownWizardStep(stepId));
  ...
  const activateTab = (tabId: string): boolean => {
    ...
    if (targetIndex < 0) throw new TypeError(viewErrorMessages.unknownTab(tabId));
