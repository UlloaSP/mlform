...
import type { FormLayoutNode } from "./layout-types";
...
export type LayoutReferenceMaps = {
  ...
  fieldSectionIds: Map<string, readonly string[]>;
  reportSectionIds: Map<string, readonly string[]>;
...
export const assertKnownField = (fieldId: string, entities: EntityMaps): void => {
  if (!entities.fieldIds.has(fieldId)) {
    throw new TypeError(viewErrorMessages.unknownFieldReference(fieldId));
  }
};

export const assertKnownReport = (reportId: string, entities: EntityMaps): void => {
  if (!entities.reportIds.has(reportId)) {
    throw new TypeError(viewErrorMessages.unknownReportReference(reportId));
  }
};
...
export const markField = (
  ...
  if (maps.fieldIdsInLayout.has(fieldId)) {
    throw new TypeError(viewErrorMessages.fieldDuplicateInLayout(fieldId));
  }
  ...
  if (sectionIds.length > 0) maps.fieldSectionIds.set(fieldId, sectionIds);
};
...
export const markReport = (
  ...
  if (maps.reportIdsInLayout.has(reportId)) {
    throw new TypeError(viewErrorMessages.reportDuplicateInLayout(reportId));
  }
  ...
  if (sectionIds.length > 0) maps.reportSectionIds.set(reportId, sectionIds);
};
...
export const assertAllFieldsCovered = (
  ...
  for (const field of fields) {
    if (!maps.fieldIdsInLayout.has(field.id)) {
      throw new TypeError(viewErrorMessages.fieldMissingInLayout(field.id));
    }
  }
};
