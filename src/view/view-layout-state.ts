...
import type { DisclosureState, FormViewState, TabsState, WizardState } from "./view-core-types";
import type { ResolvedFormLayout, ResolvedFormLayoutNode } from "./layout-types";
...
export const isVisibleInLayout = (
  ...
  if (layout.kind !== "wizard") {
    if (layout.kind === "tabs") {
      ...
      return tabId === currentTabId && sectionIds.every((id) => openSectionIds.has(id));
    }

    return sectionIds.every((id) => openSectionIds.has(id));
  }

  const currentStepId = layout.steps[stepIndex]?.id ?? null;
  return stepId === currentStepId && sectionIds.every((id) => openSectionIds.has(id));
};
