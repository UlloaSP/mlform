...
import { resolveViewRegistryPack } from "./registry-pack";
import type {
...
export const createFormView = (options: CreateFormViewOptions): FormViewController => {
  const registryPack = resolveViewRegistryPack(options);
  ...
  let resolvedLayout: ReturnType<typeof resolveFormLayout>;
  try {
    resolvedLayout = resolveFormLayout(options.layout, form.fields, form.reports);
  } catch (error) {
    form.dispose();
    throw error;
  }
