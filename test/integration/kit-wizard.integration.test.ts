...
describe("kit wizard integration", () => {
  it("mounts a wizard, validates step-by-step, submits, and renders reports", async () => {
    ...
    const nextButton = getShadow(mounted.host).querySelector(".btn-next") as HTMLButtonElement;
    nextButton.click();
    await flush();

    expect(getShadow(mounted.host).textContent).toContain("Profile");
    expect(mounted.form.getField("name")?.state.errors).toContain("This field is required.");
  ...
  it("replaces a previous mounted wizard in the same container", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    ...
    const first = mountForm(container, {
      ...
      layout: {
        kind: "wizard",
        steps: [{ title: "First", children: [{ kind: "field", field: "first" }] }],
      },
    });
    ...
    const second = mountForm(container, {
      ...
      layout: {
        kind: "wizard",
        steps: [{ title: "Second", children: [{ kind: "field", field: "second" }] }],
      },
    });
    ...
    expect(container.querySelectorAll("mlf-kit-wizard").length).toBe(1);
